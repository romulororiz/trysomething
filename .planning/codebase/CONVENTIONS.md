...
- Imports grouped in order:
  1. `package:flutter/`
  2. `package:flutter_riverpod/`
  3. Third-party packages (e.g., `package:go_router/`, `package:dio/`)
  4. Relative imports (`../../models/`, `../theme/`)
- Section comments: `// ═══════════════════════════════════` separate logical sections
- Class documentation comments: `///` precedes each public class/function
...
**Order:**
1. Flutter/Dart standard library: `import 'package:flutter/';`, `import 'dart:math';`
2. Third-party packages: `import 'package:flutter_riverpod/';`, `import 'package:dio/';`
3. Relative imports: `import '../../models/';`, `import '../theme/';`
...
**Patterns:**
- Try/catch in notifiers for API calls — see `auth_provider.dart` lines 87–111
...
```
- Separator lines for major errors:
```dart
debugPrint('══════════════════════════════════════════');
...
- Gotchas and platform-specific behavior: Note workarounds (e.g., Windows GoogleSignIn hangs)
- Section headers: Use separator comments to group related code
...
**Exports:**
- Repository pattern: Abstract interface in `hobby_repository.dart`, implementation in `hobby_repository_api.dart`
- Notifiers: Exported as class + provider in single file (e.g., `auth_provider.dart`)
- Models: `@freezed` classes with `fromJson`/`toJson` methods in single file
...
**Theme Constants:**
- Never hardcode colors: Use `AppColors.coral`, `AppColors.driftwood`
