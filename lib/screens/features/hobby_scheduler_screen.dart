...
/// Hobby Scheduler — week view with 7 day columns and scheduled hobby sessions.
class HobbySchedulerScreen extends ConsumerWidget {
  const HobbySchedulerScreen({super.key});

  static const List<String> _dayLabels = [
    'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun',
  ];

  // Time range for the grid (7:00 – 22:00)
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text('Hobby Scheduler', style: AppTypography.serifHeading),
        ],
      ),
    );
  }
  ...
  Widget _buildScheduleGrid(WidgetRef ref, List<ScheduleEvent> events) {
                ...
                children: [
                                ...
                                child: Row(
                                  children: [
                                    Icon(AppIcons.badgeTime,
                                        size: 16, color: AppColors.driftwood),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child:
                          Text('Add to schedule', style: AppTypography.sansCta),
                    ),
                  ),
                ],
              ),
...
class _ScheduleEventBlock extends ConsumerWidget {
  final ScheduleEvent event;
  final int startHour;
  final double hourHeight;

  const _ScheduleEventBlock({
    required this.event,
    required this.startHour,
    required this.hourHeight,
  });

  // Accent colors by hobby for visual distinction
  static const Map<String, Color> _hobbyColors = {
    'pottery': AppColors.coral,
    'bouldering': AppColors.indigo,
    'sourdough': AppColors.amber,
    'calligraphy': AppColors.sage,
    'guitar': AppColors.sky,
    'chess': AppColors.driftwood,
    'hiking': AppColors.sage,
    'skateboarding': AppColors.coral,
  };

  static const Map<String, Color> _hobbyBgColors = {
    'pottery': AppColors.coralPale,
    'bouldering': AppColors.indigoPale,
    'sourdough': AppColors.amberPale,
    'calligraphy': AppColors.sagePale,
    'guitar': AppColors.skyPale,
    'chess': AppColors.sand,
    'hiking': AppColors.sagePale,
    'skateboarding': AppColors.coralPale,
  };
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final accent = _hobbyColors[event.hobbyId] ?? AppColors.coral;
    final bg = _hobbyBgColors[event.hobbyId] ?? AppColors.coralPale;
